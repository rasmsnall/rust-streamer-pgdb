...
| `native_arrays` | `bool` | `False` | Map a one-dimensional array of a supported element type to a native Arrow list instead of text. See Chapter V, Section 5 |
...
The same is true of an array column `native_arrays` could not map natively. Both are
visible in the resulting Delta schema. See `architecture.md`, Chapter VIII, Section 2.
...
`native_arrays` does the same for a one-dimensional array, mapping it to a native Arrow
list instead of keeping it as PostgreSQL's own `{...}` literal text. It only ever moves a
column *toward* a native type, never toward failure: an array whose element type this
library cannot represent (a multi-dimensional declaration, an unrecognised element type,
or a `numeric` element that does not fit even `wide_numeric_as_decimal`) simply keeps
degrading to `Utf8`, decided once when the column's type is resolved rather than per
value. A `numeric` element is subject to `wide_numeric_as_decimal` exactly as a plain
`numeric` column would be, including the overflow behaviour above, applied per element.
...
| `native_arrays` | The target is Databricks and a PostgreSQL array should be a real Arrow list, not text |
